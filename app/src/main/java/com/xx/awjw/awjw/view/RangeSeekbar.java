...
public class RangeSeekbar extends View {
    ...
    private float mLeftCursorIndex = 0;//记录左边的视图位置
    private float mRightCursorIndex = 1.0f;//记录右边的视图位置
...
//    private Rect[] mClickRectArray;
    ...
    private boolean resetFlag = true;//重置标志，true为重置
...
//            mClickRectArray = new Rect[mTextArray.length];
    ...
    private void applyConfig(Context context, AttributeSet attrs) {
        ...
        num = a.getInt(R.styleable.RangeSeekbar_num, 10);
        sum = a.getInt(R.styleable.RangeSeekbar_sum, 100);
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        final int maxOfCursor = Math.max(leftPointerH/2, rightPointerH/2);//xx修改了/2
        ...
        int heightNeeded = maxOfCursorAndSeekbar + mMarginBetween + mTextSize/2//xx修改了/2
...
//        heightNeeded, MeasureSpec.EXACTLY);
        ...
        mSeekbarRect.top = mPaddingRect.top + mTextSize/2 + mMarginBetween;//xx修改了/2
...
//        }
        ...
        int left = (int) (((mLeftCursorRect.right-leftWidth - mPaddingRect. left)*sum)/(mSeekbarRect.right-mSeekbarRect.left-leftWidth))*num;
        ...
        int right = (int) (((mRightCursorRect.left-rightWidth - mPaddingRect. right) *sum)/(mSeekbarRect.right-mSeekbarRect.left-rightWidth))*num;
       
        if(left == 0 &&right ==  sum*num){
        	mPaint.setColor(mTextColorNormal);
        }

        if(left == 0){
        	float lWidth = mPaint.measureText("不限");
        	canvas.drawText("不限", mLeftCursorRect.left + mLeftCursorBG.getIntrinsicWidth()/2-lWidth/2, mSeekbarRect.top
        			- 50, mPaint);
        }else{
        	float lWidth = mPaint.measureText(left+"");
        	canvas.drawText(left+"", mLeftCursorRect.left + mLeftCursorBG.getIntrinsicWidth()/2-lWidth/2, mSeekbarRect.top
        			- 50, mPaint);
        }
        if(right == sum*num){
        	float rWidth = mPaint.measureText("不限");
        	canvas.drawText("不限", mRightCursorRect.left + mRightCursorBG.getIntrinsicWidth()/2-rWidth/2, mSeekbarRect.top
        			- 50, mPaint);
        }else{
        	float rWidth = mPaint.measureText(right+10+"");
        	canvas.drawText(right+10+"", mRightCursorRect.left + mRightCursorBG.getIntrinsicWidth()/2-rWidth/2, mSeekbarRect.top
        			- 50, mPaint);
        }
    }
...
//                }
            ...
            mRightCursorBG.setState(mUnPresseEanabledState);
            mRightPointerID = -1;
            mRightHited = false;
